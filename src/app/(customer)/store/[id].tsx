  ...
  return (
    <View style={styles.screenWrapper}>
      ...
      >
        ...
        {/* Our Services (Multi-select checkbox design) */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Our Services</Text>
          <Text style={styles.subTitleInfo}>Select one or more services to book together</Text>
          {services.length === 0 ? (
            ...
            services.map((service: any, index: number) => {
              ...
              const isExpanded = expandedServiceIds.includes(sId);
              const parsed = parseServiceMetadata(service);
              const inclusions = getServiceInclusions(parsed);
              ...
              return (
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    onPress={() => toggleServiceExpand(sId)}
                  >
                    {/* Checkbox (Taps toggle selection) */}
                    <TouchableOpacity
                    ...
                    {/* Name & Price */}
                    <View style={styles.serviceMetaTitleRow}>
                      <Text style={styles.serviceName}>{parsed.name}</Text>
                      <View style={styles.priceContainer}>
                        {parsed.originalPrice && parsed.originalPrice > parsed.price ? (
                          <Text style={styles.originalPriceText}>₹{parsed.originalPrice}</Text>
                        ) : null}
                        <Text style={styles.servicePrice}>₹{parsed.price}</Text>
                      </View>
                    </View>
                  ...
                  {isExpanded && (
                    <View style={styles.expandedDetailsPanel}>
                      {/* Description */}
                      {parsed.description ? (
                        <Text style={styles.expandedDescText}>{parsed.description}</Text>
                      ) : null}
                      ...
                      {parsed.originalPrice && parsed.originalPrice > parsed.price ? (
                        <View style={styles.discountRow}>
                          ...
                          <Text style={styles.discountText}>
                            Save ₹{parsed.originalPrice - parsed.price} ({parsed.discountPercentage || Math.round(((parsed.originalPrice - parsed.price)/parsed.originalPrice)*100)}% OFF)
                          </Text>
                        </View>
                      ) : null}
                      ...
                      <View style={styles.detailsGroup}>
                        <Text style={styles.detailsGroupTitle}>What's Included:</Text>
                        <View style={styles.inclusionsWrapper}>
                          {inclusions.map((item: string, i: number) => (
                            <View key={`${item}-${i}`} style={styles.inclusionBadge}>
                              <Ionicons name="checkmark-circle" size={14} color="#15803D" style={{ marginRight: 4 }} />
                              <Text style={styles.inclusionBadgeText}>{item}</Text>
                            </View>
                          ))}
                        </View>
                      </View>
                      ...
                      {/* Exclusions */}
                      {parsed.notIncludes && Array.isArray(parsed.notIncludes) && parsed.notIncludes.filter((item: any) => typeof item === "string" && item.trim() !== "").length > 0 && (
                        <View style={styles.detailsGroup}>
                          <Text style={styles.detailsGroupTitle}>What's NOT Included:</Text>
                          <View style={styles.exclusionsWrapper}>
                            {parsed.notIncludes.filter((item: any) => typeof item === "string" && item.trim() !== "").map((item: string, i: number) => (
                              <View key={`${item}-${i}`} style={styles.exclusionBadge}>
                                <Ionicons name="close-circle" size={14} color="#B91C1C" style={{ marginRight: 4 }} />
                                <Text style={styles.exclusionBadgeText}>{item}</Text>
                              </View>
                            ))}
                          </View>
                        </View>
                      ...
                      {/* Suitable for Breeds & Ages */}
                      {(parsed.suitableBreeds || (parsed.suitableAgeGroups && parsed.suitableAgeGroups.length > 0)) && (
                        <View style={styles.detailsGroup}>
                          <Text style={styles.detailsGroupTitle}>Suitability:</Text>
                          <View style={styles.suitabilityContainer}>
                            {parsed.suitableBreeds ? (
                              <View style={styles.suitabilityRow}>
                                <Text style={styles.suitabilityLabel}>Breeds: </Text>
                                <Text style={styles.suitabilityValue}>{parsed.suitableBreeds}</Text>
                              </View>
                            ) : null}
                            {parsed.suitableAgeGroups && parsed.suitableAgeGroups.length > 0 ? (
                              <View style={styles.suitabilityRow}>
                                <Text style={styles.suitabilityLabel}>Ages: </Text>
                                <Text style={styles.suitabilityValue}>
                                  {Array.isArray(parsed.suitableAgeGroups) ? parsed.suitableAgeGroups.join(", ") : parsed.suitableAgeGroups}
                                </Text>
                              </View>
                            ) : null}
                          </View>
                      ...
                      {/* Special Offers */}
                      {parsed.specialOffers && Object.values(parsed.specialOffers).some(v => v === true) && (
                        <View style={styles.detailsGroup}>
                          <Text style={styles.detailsGroupTitle}>Active Offers:</Text>
                          <View style={styles.offersWrapper}>
                            {parsed.specialOffers.limitedTimeOffer && (
                              <View style={styles.offerBadge}>
                                <Ionicons name="gift" size={14} color="#7E22CE" style={{ marginRight: 4 }} />
                                <Text style={styles.offerBadgeText}>Limited Time Offer</Text>
                              </View>
                            )}
                            {parsed.specialOffers.buyOneGetOne && (
                              <View style={styles.offerBadge}>
                                <Ionicons name="gift" size={14} color="#7E22CE" style={{ marginRight: 4 }} />
                                <Text style={styles.offerBadgeText}>Buy 1 Get 1 Free</Text>
                              </View>
                            )}
                            {parsed.specialOffers.packageDiscounts && (
                              <View style={styles.offerBadge}>
                                <Ionicons name="gift" size={14} color="#7E22CE" style={{ marginRight: 4 }} />
                                <Text style={styles.offerBadgeText}>Package Discount</Text>
                              </View>
                            )}
                          </View>
                        </View>
